%Main3_HSMM_4_Compressor_4_k_compare.m, 王佳琪，2018-1-15
%数据用diff
%该程序需要10核并行计算
...
%画图
save_directory = ['失速预测-k对比','重采样600重采样-P-0-29-',Ch_name(con,:),num2str(Famen{con}(hh)),'%训练样本',date];  %频谱图存储文件夹
...
for testnum=[1 3] %
plot(Loglik_con1_5_without_P{con}{testnum,1}(1,:),'r','LineWidth',2);hold on
plot(Loglik_con1_5_without_P{con}{testnum,1}(2,:),'b','LineWidth',2);hold on
plot(Loglik_con1_5_without_P{con}{testnum,1}(9,:),'k','LineWidth',2);hold on
plot(Loglik_con1_5_without_P{con}{testnum,1}(10,:),'g','LineWidth',2);hold on
hold on
end
title([Ch_name(con,:),'训练-10000rpmVS12000rpm测试-k-',num2str(kk)])
saveas(h,[directory,num2str(num(1)),'-29号特征-',num2str(Famen{con}(hh)),'%训练样本-基于HSMM的',num2str(kk),'测试样本的不同测点PI曲线图','.fig'])
saveas(h,[directory,num2str(num(1)),'-29号特征-',num2str(Famen{con}(hh)),'%训练样本-基于HSMM的',num2str(kk),'测试样本的不同测点PI曲线图','.png'])
...
function [figure1]=createfigure(YMatrix1,num,con,hh,testnum,Ch_name,Famen,figure1)
...
% 使用 plot 的矩阵输入创建多行
plot1 = plot(YMatrix1,'LineWidth',2,'LineStyle','none');
set(plot1(1),'DisplayName','B测点','Marker','+','Color',[0 0 0],...
    'LineStyle','-');
set(plot1(2),'DisplayName','R-1测点','Marker','+','Color',[0 0 1],...
    'LineStyle','-');
...
set(plot1(9),'DisplayName','R-8测点','Marker','+','Color',[1 1 0],...
    'LineStyle','-');
set(plot1(10),'DisplayName','C测点','Marker','+','Color',[1 0 0],...
    'LineStyle','-');
